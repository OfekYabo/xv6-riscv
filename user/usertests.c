...
truncate3(char *s)
{
  ...
  wait(&xstatus, exit_msg_ptr);
  unlink("truncfile");
  exit(xstatus, exit_msg_ptr);
}
...
exectest(char *s)
{
  ...
  if (wait(&xstatus, exit_msg_ptr) != pid) {
...
exitwait(char *s)
{
  ...
  for(i = 0; i < 100; i++){
    ...
    if(pid){
      int xstate;
      if(wait(&xstate, exit_msg_ptr) != pid){
...
forktest(char *s)
{
  ...
  for(; n > 0; n--){
    if(wait(0, exit_msg_ptr) < 0){
      printf("%s: wait stopped early\n", s);
      exit(1, "fail");
    }
  }

  if(wait(0, exit_msg_ptr) != -1){
